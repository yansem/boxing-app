<template>
    <header class="bg-black flex justify-end">
        <div>
            <button class="bg-yellow-400 py-2 px-4 rounded">Войти</button>
    ...
    <div v-if="isShowRegister" class="fixed inset-0 flex items-center justify-center">
        ...
        <div class="modal-content bg-white p-4 rounded-lg relative">
            ...
            <h1 class="text-2xl font-semibold">Modal Form</h1>
            <form id="modalForm" class="mt-4">
                <!-- Form inputs go here -->
                <div class="mb-4">
                    <label for="login" class="block font-medium text-gray-700">Логин</label>
                    <input v-model="registerForm.login" type="text" id="login" name="login" class="w-full p-2 border rounded-lg">
                </div>
                <div class="mb-4">
                    <label for="password" class="block font-medium text-gray-700">Пароль</label>
                    <input v-model="registerForm.password" type="password" id="password" name="password" class="w-full p-2 border rounded-lg">
                </div>
                <div class="mb-4">
                    <label for="confirm" class="block font-medium text-gray-700">Подтвердите пароль</label>
                    <input v-model="registerForm.confirm" type="password" id="confirm" name="confirm" class="w-full p-2 border rounded-lg">
                </div>
                <!-- Add more form fields as needed -->
                <button class="bg-yellow-400 py-2 px-4 rounded" @click="registration">Ок</button>
            </form>
        </div>
    </div>
...
const registration = () => {
    axios.post('/api/registration', registerForm)
        .then()
        .catch()
}
</script>
